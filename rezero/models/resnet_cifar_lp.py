...
class ResNet_s(nn.Module):
    ...
    def forward(self, x, gt = None, flag = None, cls_num_list = None):                
        ...
        if flag:
            ...
            feat = x_new.unsqueeze(dim = 1)
            obj_vec = self.obj_vector.unsqueeze(dim = 0).unsqueeze(dim = -1).unsqueeze(dim = -1)
            similar = F.cosine_similarity(obj_vec, feat, dim = 2)
            
            mask, _ = similar.max(dim = 1, keepdim = True)
