...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.loss_type == 'CE':
        ...
        criterion = BSCELoss(cls_num_list=cls_num_list).cuda(args.gpu) 
...
def validate(val_loader, model, criterion, epoch, args, log=None, tf_writer=None, flag='val'):
    ...
    all_preds = []
    all_targets = []
    feature_per_cls = dict()
    for i in range(100):
        feature_per_cls[i] = []
    # print(feature_per_cls)
    # exit()
    with torch.no_grad():
        end = time.time()
        for i, (input, target) in enumerate(val_loader):
            ...
            for i, (feature, idx) in enumerate(zip(output, target)):
                # print(feature.shape)
                mean_var = torch.cat([feature.mean().reshape(1), feature.var().reshape(1)])
                # print(mean_var.shape)
                feature_per_cls[idx].append(mean_var)
            # exit()
            # loss = criterion(output, target) 
    ...
    cls_max = []
    for i in range(100):
        # ft = torch.tensor(feature_per_cls[i])
        # print(ft.shape)
        feat = -torch.log(torch.stack(feature_per_cls[i]).T)
        
        cls_mean_mean.append(feat[0].mean())
        cls_mean_var.append(feat[0].var())
        cls_var_mean.append(feat[1].mean())
        cls_var_var.append(feat[1].var())
        # cls_min.append(feat[0].min())
        # cls_max.append(feat[0].max())
    import matplotlib.pyplot as plt
    
    plt.style.use('default')
    plt.rcParams['figure.figsize'] = (50, 20)
    
    # cw_mean = cw.mean(dim = 1, keepdim = True)
    # cw_var = cw.var(dim = 1, keepdim = True)
    fig, ax = plt.subplots()
    # ax.set_ylim(cw_mean.min()-1, cw_var.max() + 1)
    ax.set_ylim(feat.min(), feat.max())
    ax.set_xlabel('Class',fontsize=100)
    ax.set_ylabel('Value', fontsize=100)
    ax.set_xticks(np.arange(100))

    
    # for i in range(100):
    #     print(cw[i].mean(), cw[i].var(), cw[i].max(), cw[i].min())
    #     ax.violinplot(cw[i], positions = [i], showmeans=True)
    ax.plot(cls_mean_mean, label = 'mean_mean')
    ax.plot(cls_mean_var, label = 'mean_var')
    ax.plot(cls_var_mean, label = 'var_mean')
    ax.plot(cls_var_var, label = 'var_var')
    
    
    # ax.plot(cls_max, label = 'max')
    ax.legend(fontsize = 50)
    plt.savefig('feature_mean_var_mean_var.png')
    
    # for i in range(100):
    #     # i = str(i)
    #     feat = feature_per_cls[i]
    #     print(feat.mean(), feat.var(), feat.min(), feat.max())
    # # feat_mean = feature_per_cls.mean(dim = 1, keepdim = True)
    # feat_var = feature_per_cls.var(dim = 1, keepdim = True)
    # feat_min, _ = feature_per_cls.min(dim = 1, keepdim = True)
    # feat_max, _ = feature_per_cls.max(dim = 1, keepdim = True)
    # print(feat_mean, feat_var, feat_min, feat_max)
